...
description: 'Components and factory functions exported by retend-utils.'
---
...
An input component with two-way data binding. The `model` prop accepts a `Cell` whose value is synchronized with the input element. The cell type should correspond to the input type.
...
Renders a list with dynamic sizing, staggered animations, and configurable layouts. It uses FLIP-based animation for transitions between list states.
...
Animates the children of a `createUnique()` component with FLIP animation. When the unique instance moves in the tree, `UniqueTransition` animates its children from their previous position and size to their new position and size using CSS transforms.

Use this component within a `createUnique` component. See [Unique Instances](/docs/unique) for details.
